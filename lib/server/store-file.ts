...
export async function updateSessionFile(sessionId: string, updates: Partial<IntakeSession>) {
  const store = await readStore();
...
export async function updateOrderFile(orderId: string, updates: Partial<Order>) {
  const store = await readStore();
...
export async function getOrderFile(orderId: string) {
  const store = await readStore();
  return store.orders[orderId] ?? null;
}

export async function getOrderByStripeSessionIdFile(stripeSessionId: string) {
  const store = await readStore();
  return (
...
export async function listOrdersByEmailFile(email: string) {
  const store = await readStore();
...
export async function listOrdersByUserIdFile(userId: string) {
  const store = await readStore();
  return Object.values(store.orders)
...
export async function syncOwnedRecordsForUserFile(input: {
  userId: string;
  email: string;
}) {
  ...
  for (const [sessionId, session] of Object.entries(store.sessions)) {
    if (session.email?.toLowerCase() === normalizedEmail && session.userId !== input.userId) {
  ...
  for (const [orderId, order] of Object.entries(store.orders)) {
    if (order.email.toLowerCase() === normalizedEmail && order.userId !== input.userId) {
  ...
  for (const [reportId, report] of Object.entries(store.reports)) {
    if (report.email.toLowerCase() === normalizedEmail && report.userId !== input.userId) {
...
export async function createOrUpdateReportFile(report: ReportRecord) {
  const store = await readStore();
...
export async function getReportByOrderIdFile(orderId: string) {
  const store = await readStore();
  return (
...
export async function getReportBySessionAndKindFile(
  sessionId: string,
