  ...
  return (
    <Shell className="space-y-10">
      <div className="space-y-4">
        ...
        <h1 className="text-balance font-serif text-3xl text-stone-50 sm:text-6xl">
          The branch details are now open.
        </h1>
        <p className="max-w-3xl text-pretty text-base leading-7 text-stone-300 sm:text-lg sm:leading-8">
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        {previews.map((preview) => (
          <RitualCard key={preview.system} className="space-y-4">
            ...
            <h2 className="text-balance font-serif text-3xl text-stone-50 sm:text-4xl">{preview.title}</h2>
      ...
      <RitualCard className="space-y-4">
        ...
        <h2 className="text-balance font-serif text-3xl text-stone-50 sm:text-4xl">{profile.coreConclusion}</h2>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
        <Link
          href={`/report/${session.crossoverReportId ?? "demo-report"}`}
          className="rounded-full bg-stone-100 px-5 py-3 text-center text-sm font-semibold text-stone-950 transition hover:bg-cyan-100"
        >
          Open The Cross-Over Report
        </Link>
        <Link
          href={`/tracks/${session.id}`}
          className="rounded-full border border-white/10 px-5 py-3 text-center text-sm text-stone-100 transition hover:bg-white/8"
        >
